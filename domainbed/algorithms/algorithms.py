...
class ARM(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if torch.cuda.device_count() > 1:
            print('Using multi-gpu for model paralleling')
            self.context_net = nn.DataParallel(self.context_net)
...
class AbstractDANN(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, conditional, class_balance):
        ...
        if torch.cuda.device_count() > 1:
            ...
            self.class_embeddings = nn.DataParallel(self.class_embeddings)
...
class SagNet(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if torch.cuda.device_count() > 1:
            print('Using multi-gpu for model paralleling')
            self.network_f = nn.DataParallel(self.network_f)
            self.network_c = nn.DataParallel(self.network_c)
            self.network_s = nn.DataParallel(self.network_s)
