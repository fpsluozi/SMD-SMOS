...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    #######################################################
    # loading model pretrained weights from other datasets
    #######################################################
    
    # logger.info("Using algorithm: " + algorithm_class)
    if hparams['miro_pretrained']:
        logger.info("Loading pretrained "+ str(algorithm_class) + " model")
        ckpt_path = hparams['miro_pretrained_path']
        checkpoint = torch.load(ckpt_path, map_location="cpu")
        state_dict = checkpoint['model_dict']
        
        num_original_layers = len(state_dict)
        
        """
        featurizer_names = ["pre_featurizer", "featurizer", 'network.0.', 'network_f.']
        for k in list(state_dict.keys()):
            for featurizer_name in featurizer_names:
                if featurizer_name in k:
                    continue #keep the featurizer for finetuning
                else:
                    del state_dict[k]
                    break
        """
        
        linear_keywords = ["classifier", "network.1", "network_c", "network_s", "discriminator", "class_embeddings"]
        ...
        logger.info("# of layers ditched: " + str(num_original_layers - len(state_dict)))
        msg = algorithm.load_state_dict(state_dict, strict=False)
        # logger.info("state_dict_load message: " + msg) 
