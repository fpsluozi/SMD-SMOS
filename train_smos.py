...
def main():
    ...
    parser.add_argument("--test_envs", type=lambda s: [[int(item)] for item in s.split(',')], default=None)
    parser.add_argument("--aug_type", type=str, default=None)
    parser.add_argument("--downsample_size", type=int, default=None)
    parser.add_argument("--holdout_fraction", type=float, default=0.2)
    parser.add_argument("--model_save", default=2400, type=int, help="Model save start step")
    ...
    # hparams.argv_update(optimized_params)
    ...
    args.out_dir = args.out_root / args.algorithm / args.unique_name
    args.out_dir.mkdir(exist_ok=True, parents=True)

    writer = get_writer(args.out_root / "runs" / args.unique_name, args.project_name)
    ...
    if not args.test_envs:
        ...
        assert('_' in args.test_envs)
        test_envs_str = str(args.test_envs)
        test_envs_list = []
        for te_str in test_envs_str.split('_'):
            test_envs_list.append([int(te_str)])
        args.test_envs = test_envs_list
    ...
    if not args.test_envs:
        ...
        test_env_names = list()
        for te_list in args.test_envs:
            test_env_names.append(dataset.environments[te_list[0]])
        table = PrettyTable(["Selection"] + test_env_names + ["Avg."])
    for key, row in results.items():
        row.append(np.mean(row))
        row = [f"{acc:.3%}" for acc in row]
        table.add_row([key] + row)
        if key=='SWAD':
            writer.writer.add_text('SWAD_Average', str(row[-1]))
    logger.nofmt(table)
    writer.writer.add_text('Results_Table', table.get_string())
